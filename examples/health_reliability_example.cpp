...
public:
    ...
    result<std::string> execute_query(const std::string& query) {
        ...
        return result<std::string>::success("Query result for: " + query);
    }
...
public:
    result<std::string> call_api(const std::string& endpoint) {
        ...
        if (call_count_ % 3 == 0) {
            failure_count_++;
            return make_error<std::string>(
                monitoring_error_code::internal_error,
                "Internal server error"
            );
        }
        
        failure_count_ = 0;  // Reset on success
        return result<std::string>::success("API response from: " + endpoint);
    }
...
void demonstrate_circuit_breaker() {
    ...
    auto fallback = []() -> result<std::string> {
        return result<std::string>::success("Cached response (fallback)");
    };
...
void demonstrate_retry_policy() {
    ...
    auto flaky_operation = [&attempt_count]() -> result<std::string> {
        ...
        return result<std::string>::success("Operation succeeded!");
    };
    
    // Execute with retry
    std::cout << "\n1. Executing flaky operation with retry:" << std::endl;
    auto result = retry.execute(flaky_operation);
    
    if (result) {
        std::cout << "  Final result: SUCCESS - " << result.value() << std::endl;
    } else {
        std::cout << "  Final result: FAILED - " << result.get_error().message << std::endl;
    }
    
    std::cout << "  Total attempts: " << attempt_count << std::endl;
    
    // Demonstrate async retry
    std::cout << "\n2. Async retry execution:" << std::endl;
    attempt_count = 0;
    
    auto future = retry.execute_async(flaky_operation);
    
    std::cout << "  Waiting for async result..." << std::endl;
    auto async_result = future.get();
    
    if (async_result) {
        std::cout << "  Async result: SUCCESS" << std::endl;
    } else {
        std::cout << "  Async result: FAILED" << std::endl;
    }
}
...
void demonstrate_error_boundaries() {
    ...
    error_boundary boundary("critical_section", config);
    
    // Set error handler
    boundary.set_error_handler([](const error_info& error) {
        std::cout << "  Error handler called: " << error.message << std::endl;
    });
    ...
    for (int i = 1; i <= 7; ++i) {
        auto result = boundary.execute<std::string>([i]() -> result<std::string> {
            ...
            std::cout << "SUCCESS" << std::endl;
            return result<std::string>::success("Result " + std::to_string(i));
        });
