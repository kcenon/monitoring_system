...
### 변경됨
- **컴파일러 최소 버전 상향 조정: GCC 13+/Clang 17+/MSVC 2022+/Apple Clang 14+** — `thread_system` 전이 의존성 요구사항 반영. 아래 `[4.0.0]` 항목의 GCC 10+/Clang 10+/MSVC 2019 16.3+ 기록은 과거 상태이며, 권위 있는 최소 버전은 `README.md` 및 `CLAUDE.md`에 정의됩니다.
...
- **공용 ring_buffer.h 및 metric_storage.h API** (#339)
  ...
  - ring buffer 버퍼링과 시계열 스토리지를 갖춘 `metric_storage` 클래스
  - 백그라운드 처리 지원이 포함된 스레드 안전 구현
  - 용량, 보존 기간, flush 간격 설정이 가능한 `metric_storage_config`
  - 스토리지 성능 모니터링을 위한 `metric_storage_stats`
  - 14개의 테스트가 통과하는 `test_metric_storage.cpp` 활성화
...
- **예제 파일들을 현재 Result<T> API에 맞게 업데이트** (#326)
  ...
  - `logger_di_integration_example.cpp` 수정: performance_monitor API 업데이트 (record_counter, record_gauge, record_histogram, collect), ILogger 인터페이스 구현 수정
  - 모든 예제가 현재 API로 컴파일 및 동작
...
상세한 마이그레이션 지침은 [ARCHITECTURE_GUIDE.md](advanced/ARCHITECTURE_GUIDE.md)를 참조하세요.
...
- **Security**: 보안 관련 문제는 [SECURITY.md](guides/SECURITY.md)를 참조하세요
