...
| A.8.15 | Logging | Performance monitor and collectors emit structured metrics; the library integrates with `logger_system` for operational logs and produces tracing spans with correlation IDs. | `core/performance_monitor.h`, `tracing/distributed_tracer.h`, `adapters/logger_to_monitoring_adapter.h` |
