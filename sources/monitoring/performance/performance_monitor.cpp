...
monitoring_system::result<performance_metrics> performance_profiler::get_metrics(
    ...
    if (it == profiles_.end()) {
        return monitoring_system::make_error<performance_metrics>(
...
monitoring_system::result<bool> performance_profiler::clear_samples(
    const std::string& operation_name) {
...
monitoring_system::result<system_metrics> system_monitor::get_current_metrics() const {
    if (!impl_) {
        return monitoring_system::make_error<system_metrics>(
...
monitoring_system::result<bool> system_monitor::start_monitoring(
...
monitoring_system::result<bool> system_monitor::stop_monitoring() {
    if (!impl_) {
        return monitoring_system::make_error<bool>(
            monitoring_error_code::invalid_state
        );
...
monitoring_system::result<metrics_snapshot> performance_monitor::collect() {
...
monitoring_system::result<bool> performance_monitor::check_thresholds() const {
    auto sys_result = system_monitor_.get_current_metrics();
    if (!sys_result) {
        return monitoring_system::make_error<bool>(sys_result.get_error().code);
    }
    ...
    return monitoring_system::result<bool>(threshold_exceeded);
}
