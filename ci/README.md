...
```sh
python3 scripts/check_tag_reality.py --repo kcenon/monitoring_system --tag v1.2.3 \
  --release-commit FULL_RELEASE_MERGE_SHA \
  --port-dir vcpkg-ports/kcenon-monitoring-system
```
...
- `MONITORING_WITH_COMMON_SYSTEM` → `KCENON_WITH_COMMON_SYSTEM`.
- `MONITORING_WITH_THREAD_SYSTEM` → `KCENON_WITH_THREAD_SYSTEM`.
- `MONITORING_WITH_LOGGER_SYSTEM` → `KCENON_WITH_LOGGER_SYSTEM`.
- `MONITORING_WITH_NETWORK_SYSTEM` → `KCENON_WITH_NETWORK_SYSTEM`.
